...
class TrackInfo
{
	...
	clone() {
		...
		for (let i=0;i<this.encodings.length;++i)
		{
			...
			for (let j=0; this.endodings[i].length;++j)
				//Append it
				alternatives.push(this.encodings[i][j].cloned());
			//Clone and add grou
...
TrackInfo.expand = function(plain)
{
	...
	//Gor each ssrc
	for (let i=0;i<plain.ssrcs.length;++i)
		//Add ssrc
		trackInfo.addSSRC(plain.ssrcs[i]);
	//For each group
	for (let i=0;i<plain.groups.length;++i)
		//Clone and add grou
		trackInfo.addSourceGroup(SourceGroupInfo.expand(plain.groups[i]));
	//For each encoding
	for (let i=0;i<plain.encodings.length;++i)
	{
		const alternatives = [];
		//For each alternative
		for (let j=0; plain.endodings[i].length;++j)
			//Append it
			alternatives.push(TrackEncodingInfo.expand(plain.encodings[i][j]));
		//Clone and add grou
		trackInfo.addAlternativeEncoding(alternatives);
	}
	//Return it
	return trackInfo;
};
