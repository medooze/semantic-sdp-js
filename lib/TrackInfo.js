...
class TrackInfo
{
	 ...
	 * @param {String | undefined} mediaId		- MediaInfo id
	 */
	setMediaId(mediaId)
	{
...
TrackInfo.expand = function (plain)
{
	...
	else if (plain.ssrcs?.media !== undefined)
	{
		//Get ssrc for media
	...
	//If we also need to asociate an rid
	if (plain.rid)
	{
		//Create encoding
		const encoding = new TrackEncodingInfo(plain.rid);
		//If got named ssrcs
		const mediaSSRC = plain.ssrcs?.media;
		if (mediaSSRC !== undefined)
			//Add ssrc
			encoding.addParam("ssrc", mediaSSRC)
		//Add to encodngs
		trackInfo.addAlternativeEncodings([
			encoding
		]);
	}
