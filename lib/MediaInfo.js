		...
		if (supported)
		{
			...
			if (supported.codecs)
			{
				...
				if (Array.isArray(supported.codecs))
					//Generate set
					supportedCodecs = CodecInfo.MapFromNames(supported.codecs,supported.rts,supported.rtcpfbs);
				else
					//It is a set
				...
				for (let codec of this.codecs.values())
				{
					...
					{
						//Clone codec
						const cloned = supportedCodecs.get(codec.getCodec().toLowerCase()).clone();
...
MediaInfo.create = function(type,supported)
{
       ...
       if (supported)
       {
		...
		if (supported.codecs)
		{
			...
			{
				//Add all codecs
				mediaInfo.setCodecs(CodecInfo.MapFromNames(supported.codecs,supported.rts,supported.rtcpfbs));
			 } else {
				 //Add codecs
...
MediaInfo.expand = function(plain)
{
	...
	//For each codec
	for (let i=0; i<plain.codecs.length;++i)
	{
		//Parse codec
