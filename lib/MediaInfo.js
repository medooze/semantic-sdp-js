			...
			if (supported.codecs)
			{
				...
				for (let codec of this.codecs.values())
				{
					...
					if (supported.codecs.has(codec.getCodec().toLowerCase()))
					{
						...
						for (let j=0;supported.rtcpfbs && j<supported.rtcpfbs.length;++i)
							//Add rtcp feednack
							codec.addRTCPFeedback(new RTCPFeedbackInfo(supported.rtcpfbs[j].id, supported.rtcpfbs[j].params));
						//Add to answer
