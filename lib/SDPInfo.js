...
SDPInfo.create = function(params) 
{
	...
	for (let i in params.capabilities)
		...
		sdp.addMedia(MediaInfo.create(i,params.capabilities[i]));
	//Done
	return sdp;
};
...
SDPInfo.expand = function(plain)
{
	...
	//For each media
	for (let i=0;i<plain.medias.length;++i)
	{
		//Expand media
	...
	//For each stream
	for (let i=0;i<plain.streams.length;++i)
	{
		//Expand stream
	...
	//For each candiadte
	for (let i=0;i<plain.candidates.length;++i)
	{
