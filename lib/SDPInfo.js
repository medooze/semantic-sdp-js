...
class SDPInfo
{
	 ...
	 */
	getTracksByMediaType(type)
	{
		let tracks = [];
		for (const stream of this.streams.values())
			for (const [trackId,track] of stream.getTracks())
				if (equalsIgnoreCase(track.getMedia(),type))
					tracks.push(track);
		return tracks;
	}
...
SDPInfo.create = function(params) 
{
	...
	//If we have associated streams
	if (params.streams)
		//For each one
		for (const stream of params.streams)
			//Add stream clone
			sdp.addStream(StreamInfo.expand(stream));
	...
	{
		//If doinf unified
		if (params.unified && params.streams?.length)
		{
			//For each track of the media type
			for (const track of sdp.getTracksByMediaType(mediaType))
			{
				//Create media
