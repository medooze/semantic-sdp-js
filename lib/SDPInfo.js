				...
				if (send && send.length)
				{
					...
					for (let j=0;j<send.length;++j)
					{
						...
						for (let k=0;k<send[j].length;++k)
							//Add it
							alternatives += (alternatives.length ? "," : "") + (send[j][k].isPaused() ? "~" : "") + send[j][k].getRID();
				...
				if (recv && recv.length)
				{
					...
					for (let j=0;j<recv.length;++j)
					{
						...
						for (let k=0;k<recv[j].length;++k)
							//Add it
							alternatives += (alternatives.length ? "," : "") + (recv[j][k].isPaused() ? "~" : "") + recv[j][k].getRID();
						//Add stream alternatives
						list += (list.length ? ";" : "") + alternatives;
					}
					//Set attributes
