...
class SDPInfo
{
	...
	replaceMedia(media)
	{
		//Find index where media is stored
		const i = this.medias.findIndex(m => m.getId() === media.getId());
		//Replace if found
		if (i !== -1)
			this.medias[i] = media;
		//Return result
	 ...
	 * @returns {DTLSInfo | null} DTLS info object
	 */
	getDTLS() {
		return this.dtls;
	}
	 ...
	 * @param {DTLSInfo | null}  dtlsInfo - DTLS info object
	 */
	setDTLS(dtlsInfo) {
		this.dtls = dtlsInfo;
	}
	 ...
	 * @returns {CryptoInfo | null} DTLS info object
	 */
	getCrypto() {
		return this.crypto;
	}
	 ...
	 * @param {CryptoInfo | null}  cryptoInfo - DTLS info object
	 */
	 ...
	 * @returns {ICEInfo | null} ICE info object
	 */
	getICE() {
		return this.ice;
	}
	 ...
	 * @param {ICEInfo | null} iceInfo - ICE info object
	 */
	setICE(iceInfo) {
		this.ice = iceInfo;
	}
	 ...
	 * @returns {TrackInfo | null} Track info
	 */
	...
	toString()
	{
		...
		if (this.ice !== null && this.ice.isLite())
		...
		for (const media of this.medias)
		{
			...
			{
				//Set ice properties
				md.iceUfrag = this.ice.getUfrag();
				md.icePwd   = this.ice.getPwd();
			}
