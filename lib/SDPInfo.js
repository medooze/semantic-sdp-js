...
const SDESInfo		 = require("./SDESInfo");
...
class SDPInfo
{
	...
	clone() {
		...
		cloned.setSDES(this.sdes.clone());
	...
	answer(params) {
		...
		if (params.sdes)
		{
			if (params.sdes instanceof SDESInfo)
				answer.setSDES(params.sdes);
			else
				answer.setSDES(SDESInfo.expand(params.sdes));
		}
	...
	toString()
	{
		...
		for (let i in this.medias)
		{
			...
			} else if (this.getSDES()) {
				md.crypto = [{
					id : this.getSDES().getTag(),
					suite : this.getSDES().getSuite(),
					config : this.getSDES().getKeyParams()
				}];
			}
...
SDPInfo.expand = function(plain)
{
	...
	if (plain.sdes)
		sdesInfo.setSDES(DTLSInfo.expand(plain.sdes));
...
SDPInfo.parse = function(string)
{
	...
	for (let i in sdp.media)
	{
		...
		if (fingerprintAttr) {
			...
			sdpInfo.setSDES(new SDESInfo(crypto.id, crypto.suite, crypto.config, crypto.sessionConfig))
		}
